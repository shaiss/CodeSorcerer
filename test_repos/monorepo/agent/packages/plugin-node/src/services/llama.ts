                    ...
                    https
                        .get(url, (response) => {
                            ...
                            response.on("data", (chunk) => {
                                ...
                                progressString = `Downloading model: [${dots.padEnd(
                                    20,
                                    " "
                                )}] ${progress}%`;
    ...
    private async getCompletionResponse(
        ...
        if (useGrammar) {
            ...
            if (!jsonString) {
                ...
                jsonString = response.match(/\{[\s\S]*\}/)?.[0]?.trim();
            }
            ...
            try {
                ...
                elizaLogger.error("Response was:", response);
                elizaLogger.error("Extracted JSON string was:", jsonString);
                throw error;
            }
        } else {
            await this.sequence.clearHistory();
